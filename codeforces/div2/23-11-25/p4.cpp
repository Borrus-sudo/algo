...
#define pi(...)      pi __VA_ARGS__; in(__VA_ARGS__);
#define pll(...)     pll __VA_ARGS__; in(__VA_ARGS__);

#define vi(m, ...)                         vi m = move(vector<int>(__VA_ARGS__)); in(m);
#define vll(m, ...)                        vll m = move(vector<ll>(__VA_ARGS__)); in(m);

#define vvi(m, rows, cols)  vvi m(rows, vi(cols)); in(m);
#define vvll(m, rows, cols) vvll m(rows, vll(cols)); in(m);

#define str(s) string s; in(s);

#define ret(...)      out(__VA_ARGS__); return;
