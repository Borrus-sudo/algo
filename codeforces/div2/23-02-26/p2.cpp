...
bool possible(string s, int a, int b) {
    // s will be even
    // I have to take a at first, then I can take whatever the hell I want
    int n = len(s);
    ...
    for (int i = 1; i < n; i++) {
        ...
        if ((dp[i - 1][a] == 1 && dp[i - 2][b] == 1) || (dp[i - 1][b] == 1)) {
            dp[i][a] = 1;
        }
        if ((dp[i - 1][b] == 1 && dp[i - 2][a] == 1) || (dp[i - 1][a] == 1)) {
            dp[i][b] = 1;
        }
        if (s[i] == '?')
            continue;
