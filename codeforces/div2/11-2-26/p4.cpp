...
void solve() {
    ...
    vi(a, n);
    for (int i = 1; i <= n - 1; i++) {
        // factorize every number, both exist, find indices, and make sure that they differ by 1
        for (int j = 1; j * j <= i; i++) {
            if (i % j == 0) {
                int j_prime = i / j;
                if (a.has(j) && a.has(j_prime)) {
                    // find the number of indices such that ij - ij_prime = i
                }
            }
        }
    }
