...
void dfs(int v, vvi& adjm, vec<bool>& visited, vi& pres, int& ans, int m, int streak = 0) {
    dbg(v);
    visited[v] = true;
    int ns = 0;
    for (int child : adjm[v]) {
        if (visited[child]) {
            ns++;
            continue;
        }
        ...
        dfs(child, adjm, visited, pres, ans, m, nS);
    }
    if (ns == len(adjm[v])) {
        dbg("Leaf", v);
        ans++;
...
void solve() {
    ...
    int streak = pres[1] == 1;
    dbg(streak);
    dfs(1, adjm, visited, pres, ans, m, streak);
    out(ans);
}
