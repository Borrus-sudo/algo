...
void solve() {
    ll(n, m);
    // decompose the number and figure out things?
    auto expVal = poly(int n, int k) {
        int ans = 0;
        while (n % k == 0) {
            n /= k;
            ans++;
    ...
    int p5 = expVal(n, 5);
    dbg(p2, p5);
    int leftover = max(p2, p5) - min(p2, p5);
    int base = p2 >= p5 ? 5 : 2;
    if (!leftover) {
        // so this is what I called cooked ig
        int p10 = log10(m);
        ll times = pow(10ll, p10);
        ll prollyLeft = m / times;
        times *= prollyLeft;
        ret(n * times);
    }
    // there is leftover of a certain base
    // we fill it up, till we can't get no-more
    ll times = 1;
    while (times * base <= m && leftover > 0) {
        times *= base;
        leftover--;
    }
    while (times * 10 <= m) {
