...
void solve() {
    int(n);
    // do breadth first search for that
    vvi adj_m(n + 1, vec<int>());
    rep(i, n - 1) {
        int u, v;
        in(u, v);
        // dbg(u, v);
    ...
    while (!next.empty()) {
        ...
        rep(i, lvlS) {
            int node = next.front();
            partitions[idx].insert(node);
            next.pop();
            for (int neighbour : adj_m[node]) {
                if (!partitions[1 - idx].contains(neighbour))  // neighbours check ofc
                    next.push(neighbour);
            }
        }
        idx = 1 - idx;
    ...
    for (int i = 1; i <= n; i++) {
        int nS = adj_m[i].size();
        int opp = partitions[0].contains(i) ? 1 : 0;
        ans += (partitions[opp].size() - nS);
    }
    ans /= 2;
    out(ans);
}
