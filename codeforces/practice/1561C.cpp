...
void solve() {
    ...
    rep(i, n) {
        ...
        rep(j, ki) {
            int(monster);
            caves[i].push_back(monster);
        }
    }
    auto realPower =
        caves | transform(poly(auto a) {
            return zip(fill(0), a) | transform(uni(get<1>(a) - get<0>(a) + 1)) | to<int>();
        }) |
        to<vi>();
    ...
    loop(cave, realPower) {
        int powerNeeded = maxe(cave);
        int powerGained = len(cave);
        chain.push_back({powerNeeded, powerGained});
    }

    sort(chain,
         bi(a.first == b.first ? a.second > b.second
                               : a.first < b.first));  // least powerNeeded first and if powerNeeded
                                                       // tied, then by powerGained increasing

    auto [ans, gained] = chain.front();
    int check = ans + gained;
    loop(pr, chain | drop(1)) {
        auto [needed, gained] = pr;
        if (check >= needed) {
            check += (gained);
        } else {
            // some deficit, we need to sort this deficit by giving some
            ans += (needed - check);
            check += (needed - check);
            check += (gained);
        }
