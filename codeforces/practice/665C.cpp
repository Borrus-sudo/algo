...
void solve() {
    str(s);
    str res;
    cloop(grp, s | izip | chunk_by(bi(get<1>(a) == get<1>(b)))) {
        dbg(grp.size());
        if (grp.size() == 1) {
            auto [idx, letter] = grp.front();
            res += letter;
            continue;
        }
        if (grp.size() & 1) {
            // no issues here ig
            auto [f, letter] = grp.front();
            char replace = (char)(((letter - 'a') + 1) % 26 + 97);
            for (auto [idx, letter] : grp) {
                if ((idx - f + 1) & 1) {
                    res += letter;
                } else {
                    res += replace;
            ...
            for (auto [idx, letter] : grp) {
                if ((idx - f + 1) & 1) {
                    ...
                    if (idx == s.size() - 1 || s[idx + 1] != replace) {
                        res += replace;
                    } else {
                        // edge case character
                        char toReplace = s[idx + 1];
                        replace = (char)(((toReplace - 'a') + 1) % 26 + 97);
                        res += replace;
                        dbg(res);
                    }
                }
