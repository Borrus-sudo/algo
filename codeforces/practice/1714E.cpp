...
void solve() {
    int(n);
    vi(a, n);
    auto converted = a | transform(uni(conv(a))) | to<int>();
    dbg(converted);
    bool yes2 = all(converted, uni(a % 10 == 2 && abs(a - f(converted)) % 20 == 0));
    bool yes10 = all(converted, uni(a == f(converted)));
    out(yes2 || yes10 ? "YES" : "NO");
}
