...
void solve() {
    ...
    ll ans = 0;
    int total = 0;
    cloop(grp, s | enumerate | chunk_by(bi(get<1>(a) == get<1>(b)))) {
        auto [left, _] = grp.front();
        auto [right, __] = grp.back();
        if (_ == 'o') {
            // total = 0;
            continue;
        }
        total += len(grp) - 1;
        pref_cnt[right] = total;
    }
    total = 0;
    cloop(grp, rev(s | enumerate | chunk_by(bi(get<1>(a) == get<1>(b))))) {
        auto [left, _] = grp.front();
        auto [right, __] = grp.back();
        if (_ == 'o') {
            // total = 0;
    ...
    cloop(grp, s | enumerate | chunk_by(bi(get<1>(a) == get<1>(b)))) {
        ...
        if (left == 0 || right == n - 1)
            continue;
        ans += (len(grp) * 1ll * pref_cnt[left - 1] * suff_cnt[right + 1]);
    }
    out(ans);
}
