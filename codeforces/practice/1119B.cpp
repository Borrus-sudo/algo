...
void solve() {
    ...
    auto canFit = poly(int k) {
        int avail = h;
        auto firstK = a | take(k) | to<int>();
        desc(firstK);
        auto firstKOdd =
            firstK | enumerate | filter(uni(!(get<0>(a) & 1))) | views::transform(uni(get<1>(a)));
        dbg(firstKOdd | to<int>());
        auto sum = accumulate(firstKOdd, 0ll);
        return sum <= h;
    };
