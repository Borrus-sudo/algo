...
ll s2(ll n, ll m, ll k) {
    return (min({(m * 2 + 2) / 3, (m + k) / 2, n - 1, (n + m - k + 1) / 2}) + 1);
}
...
int main() {
    ...
    for (long long iter = 1;; iter++) {
        ll n = rng() % 100000 + 1;      // 1 ≤ n ≤ 1e5
        ll k = 1;                       // 1 ≤ k ≤ n
        ll m = rng() % 1000000000 + 1;  // 1 ≤ m ≤ 1e9
