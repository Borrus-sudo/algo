...
void solve() {
    ...
    // We gotta try both
    auto [a, b] = biggestFactor(n);
    if (a == -1) {
        ret("NO");
    }
    auto [a1, a2] = biggestFactor(a);
    auto [b1, b2] = biggestFactor(b);
    if (a1 != -1) {
        if (a1 == a2) {
            // more splits
            auto [a21, a22] = biggestFactor(a2);
            if (a21 != -1) {
                a1 *= a22;
                a2 = a21;
            }
