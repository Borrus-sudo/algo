...
void solve() {
    ...
    for (auto [a, b] : c) {
        if (a == b) {
            out(plus == minus ? "YES" : "NO");
            continue;
        }
        // a(plus - x)  - b(minus - x) = 0
        // a(plus) -ax = bminus -bx
        // a(plus) - b(minus) = ax - bx
        // a(plus) - b(minus) / (a - b) = x
        auto yes = poly() {
            ll quotient = (a * plus - b * minus) / (a - b);
            ll rmndr = (a * plus - b * minus) % (a - b);
            return rmndr == 0 && quotient >= 0 && quotient <= min(plus, minus);
        };
        if (yes()) {
            out("YES");
            continue;
