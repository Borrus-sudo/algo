...
// Useful methods: chunk (disjoint array), chunk_by, there is slide (but I prefer adj!)
...
void solve() {
    int(n);
    vi(a, n);
    // if there are 3 consecutive ones, that means we will use one point
    int points = f(a) == 1;
    for (auto part : a | drop(1) | chunk_by(bi(a == b))) {
        if (part.front() == 1) {
            points += len(part) / 3;
        }
    }
