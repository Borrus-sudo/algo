...
void solve() {
    pi knight, king, queen;
    in(knight, king, queen);
    // now generate all possibilities
    if (king.first == queen.first) {
        int dist = abs(king.second - queen.second);
        if (dist == min(knight.first, knight.second)) {
            if (max(knight.first, knight.second) == 1) {
                ret(1);
            } else {
                ret(2);
            }
    ...
    if (king.second == queen.second) {
        int dist = abs(king.first - queen.first);
        if (dist == min(knight.first, knight.second)) {
            if (max(knight.first, knight.second) == 1) {
                ret(1);
            } else {
    ...
    if (abs(king.first - queen.first) == abs(king.second - queen.second)) {
        int dist = abs(king.first - queen.second) - 1;
        if (dist == max(knight.first, knight.second)) {
            if (dist == 1) {
                ret(1);
            } else {
                ret(2);
