...
void solve() {
    ...
    vi(c, n);
    pfx(c);
    // find all the points where the capital would be moved
    ll curr_pos = 0;
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        // we always first have to conquer this location.
        ans += a * (c[i] - curr_pos);
        // now is it worth the cost to move the capital here?
        ll new_pos = c[i];
        ll rng_remaining = pref_c[n - 1] - pref_c[i];
        // (n - 1) - (i + 1) + 1 = n - i - 1
        ll old = (rng_remaining - (n - i - 1) * curr_pos);
        ll future = (rng_remaining - (n - i - 1) * new_pos);
        if (a * old > a * future + b * (new_pos - curr_pos)) {
            ans += b * (new_pos - curr_pos);
            curr_pos = c[i];
        }
    }
    out(ans);
