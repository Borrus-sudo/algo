...
void solve() {
    ...
    vi dp(n + 1, INT_MAX);
    si noRepeats(iall(a));
    for (int elem : noRepeats) {
        dp[elem] = 1;
        for (int multiple = 2 * elem; multiple <= n; multiple += elem) {
            if (dp[multiple / elem] != INT_MAX)
                smin(dp[multiple], dp[multiple / elem] + 1);
        }
    };
    out(dp | drop(1) | transform(uni(a == INT_MAX ? -1 : a)));
};
