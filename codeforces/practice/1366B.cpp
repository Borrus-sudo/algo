...
void solve() {
    ...
    vec<pi> r_ops(m);
    in(r_ops);
    auto ops = r_ops | drop_while(uni(!(a.first <= x && x <= a.second)));
    if (!ops.size()) {
        ret(1);
    }
    pi unionized = ops.front();
    loop(op, ops | drop(1)) {
        if ((op.first <= unionized.first && op.second >= unionized.first) ||
            (op.first <= unionized.second && op.second >= unionized.second)) {
            unionized.first = min(op.first, unionized.first);
            unionized.second = max(op.second, unionized.second);
        }
    }
    out(unionized.second - unionized.first + 1);
}
