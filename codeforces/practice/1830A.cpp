...
void dfs(int node, vvi& relations, vi& parents, vec<bool>& visited) {
    visited[node] = true;
    iter(child, relations[node]) {
        if (!visited[child]) {
            parents[child] = node;
            dfs(child, relations, parents, visited);
        }
    }
}
void solve() {
    ...
    dfs(1, relations, getParent, visited);
    vec<bool> paid(n + 1, false);
    vec<bool> disc(n + 1, false);
    disc[1] = true;
    int ans = 1;
    dbg(getParent);
    for (auto [u, v] : e) {
