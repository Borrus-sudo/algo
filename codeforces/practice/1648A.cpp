...
#define vvi(m, rows, cols)       \
    vvi m(rows, vec<int>(cols)); \
    in(m);
#define vvll(m, rows, cols)      \
    vvll m(rows, vec<ll>(cols)); \
    in(m);
#define str(s) \
    str s;     \
    in(s);
...
void solve() {
    int(n, m);
    vvi(a, n, m);
    vec<vec<pi>> colors(1e5 + 1);
    citer(row, a | izip) {
        int r = f(row);
        citer(col, b(row) | izip) {
            int c = f(col);
            int color = b(col);
            colors[color].pb({r, c});
        }
    ...
    ll res = 0;
    for (int i = 1; i <= len(colors); i++) {
        auto colorCells = colors[i];
        sort(colorCells, bi(f(a) < f(b)));
        for (auto [idx, coords] : colorCells | izip) {
            res += (f(coords) * (idx) - (f(coords) * (len(colorCells) - idx - 1)));
        };
        sort(colorCells, bi(a.second < b.second));
        for (auto [idx, coords] : colorCells | izip) {
            res += (b(coords) * (idx) - (b(coords) * (len(colorCells) - idx - 1)));
        };
    }
    out(res);
};
