...
void solve() {
    int(n);
    vll(tea, n);     // qty of tea
    vll(taster, n);  // qty of taster
    pfx(taster);    // 1 indexed
    vi pref_times(n);
    vll ans(n);
    for (auto [idx, qty] : tea | izip) {
        qty += pref_taster[idx];
        auto loc = upper_bound(pref_taster.begin() + idx + 1, pref_taster.end(), qty);
        pref_times[idx]++;
        if (loc != pref_taster.end()) {
            int end_effect = loc - pref_taster.begin();
            end_effect--;
            pref_times[end_effect]--;
            loc--;  // loc can never pref_taster.begin() to start with
            ans[end_effect] += (qty - (*loc));
        }
    }
    for (int i = 1; i < n; i++) {
        pref_times[i] += pref_times[i - 1];
    }
    for (int i = 0; i < n; i++) {
        ans[i] += ((ll)pref_times[i] * taster[i]);
    }
    out(ans);
}
