...
void solve() {
    int(n);
    vi(su, n);        // university of the ith student
    vi(ss, n);        // skill of the ith student
    vll team(n + 1);  // well welll well
    vvi u(n + 1);
    for (auto [idx, uni] : su | izip)
        u[uni].pb(ss[idx]);
    for (auto studs : u | drop(1)) {
        sort(studs);
        pfx(studs);
        int uni_size = len(studs);
        ll maxNo = pref_studs[uni_size];
        for (int k = 1; k <= uni_size; k++) {
            int removeIdx = uni_size % k;
            ll thisTeam = maxNo - (pref_studs[removeIdx]);
            team[k] += thisTeam;
        }
    }
    out(team | drop(1));
